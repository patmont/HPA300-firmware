...
#define TOUCH_CHANNEL_NUM 6
#define TOUCH_CHAN_INIT_SCAN_TIMES  3

ESP_STATIC_ASSERT(TOUCH_CHANNEL_NUM <= (TOUCH_MAX_CHAN_ID - TOUCH_MIN_CHAN_ID + 1), "touch channel number exceed the max supported number ");

// Active threshold to benchmark ratio. (i.e., touch will be activated when data >= benchmark * (1 + ratio))
static float s_thresh2bm_ratio[TOUCH_CHANNEL_NUM] = {
    [0 ... TOUCH_CHANNEL_NUM - 1] = 0.015f,  // 1.5%
};
...
bool touch_on_active_callback(touch_sensor_handle_t sens_handle, const touch_active_event_data_t *event, void *user_ctx)
{
    ESP_EARLY_LOGI("touch_callback", "[CH %d] active", (int)event->chan_id);
    return false;
}

bool touch_on_inactive_callback(touch_sensor_handle_t sens_handle, const touch_inactive_event_data_t *event, void *user_ctx)
{
...
void app_main(void)
{
    ...
    /* Step 3: Configure the default filter for the touch sensor (Note: Touch V1 uses software filter) */
